...
bool function CreateThermiteWallSegment( entity projectile, int projectileCount, entity inflictor, entity movingGeo, vector pos, vector angles, int waveCount )
{
	...
	if ( projectile.proj.savedOrigin != < -999999.0, -999999.0, -999999.0 > )
	{
		...
		if ( !movingGeo )
		{
			thermiteParticle = CreateThermiteTrail( pos, angles, owner, inflictor, duration, FLAME_WALL_FX, FLAME_WALL_FX_2, damageSource )
		...
		else
		{
			...
			{
				thermiteParticle = CreateThermiteTrailOnMovingGeo( movingGeo, pos, angles, owner, inflictor, duration, FLAME_WALL_FX, FLAME_WALL_FX_2, damageSource )
			}
